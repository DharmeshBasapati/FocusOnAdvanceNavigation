...
class HomeViewModel(private val focusDao: FocusDao) : ViewModel() {
    ...
    private fun fetchProducts() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (productsListFromDB.isEmpty()) {
                Log.d(
                    "TAG",
                    "fetchProducts: Database is Empty...so fetching from API or Static List"
                )
                ...
                val productsListFromAPIorStatic = getStaticProductsList()

                val productListToInsertInDB = mutableListOf<Products>()
                ...
                productsListFromAPIorStatic.forEach { product ->
                    ...
                    productListToInsertInDB.add(productToInsert)
                ...
                Log.d("TAG", "fetchProducts: Taking data from Database")

                products.postValue(Resource.success(productsListFromDB))
            }
    ...
    fun getProducts(): LiveData<Resource<List<Products>>> = products


    private fun getStaticProductsList(): List<Product> {
        val productResponse =
